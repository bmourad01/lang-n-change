...
module Term = struct
  ...
  let rec unbind_rec t =
    match t with
    | Wildcard | Nil | Var _ | Str _ -> t
    | Constructor {name; args} ->
  ...
  let rec unticked t =
    let f v =
      ...
      | None -> v
      | Some len -> String.sub v ~pos:0 ~len
    in
    match t with
    | Wildcard | Nil | Str _ -> t
    | Var v -> Var (f v)
  ...
  let rec substitute t sub =
    match List.Assoc.find sub ~equal t with
    | Some t' -> t'
    | None -> (
  ...
  let uniquify_map include_bindings underscore min t =
    ...
    let vars' =
      List.fold vars ~init:[] ~f:(fun vars t ->
          if List.mem vars t ~equal then vars else t :: vars)
      |> List.rev
    in
  ...
  let uniquify' ?(include_bindings = false) t m =
    let make_var t m =
      match List.Assoc.find m t ~equal with
      | None -> (t, m)
      | Some ts -> (
          let m = List.Assoc.remove m t ~equal in
          match List.length ts with
          | n when n > 1 -> (List.hd_exn ts, (t, List.tl_exn ts) :: m)
          | 0 -> (t, m)
          | _ -> (List.hd_exn ts, m) )
    in
    let rec aux t m =
  ...
  let uniquify ?(include_bindings = false) ?(underscore = true) ?(min = 1) t
      =
...
module Formula = struct
  ...
  let rec to_string = function
    ...
    | Prop {predicate; args} ->
        ...
        if Predicate.(equal predicate Builtin.typeof) && len = 3 then
        ...
        then
          ...
          Printf.sprintf "%s |- %s ==> %s : %s"
            ...
            (Term.to_string (List.nth_exn args 2))
            (Term.to_string (List.last_exn args))
        else if Predicate.(equal predicate Builtin.step) && len = 2 then
          Printf.sprintf "%s --> %s"
            (Term.to_string (List.hd_exn args))
            (Term.to_string (List.last_exn args))
        else if Predicate.(equal predicate Builtin.subtype) && len = 2 then
          Printf.sprintf "%s <: %s"
            (Term.to_string (List.hd_exn args))
            (Term.to_string (List.last_exn args))
        else if Predicate.(equal predicate Builtin.subtype) && len = 3 then
          Printf.sprintf "%s |- %s <: %s"
            (Term.to_string (List.hd_exn args))
            (Term.to_string (List.nth_exn args 1))
            (Term.to_string (List.last_exn args))
        ...
        then
          ...
          in
          Printf.sprintf "%s %s" predicate args_str
    | Member {element; collection} ->
        Printf.sprintf "member %s %s" (Term.to_string element)
          (Term.to_string collection)
    | Subset {sub; super} ->
        Printf.sprintf "subset %s %s" (Term.to_string sub)
  ...
  let rec substitute f sub =
    ...
    | Subset s ->
        Subset
          {sub= Term.substitute s.sub sub; super= Term.substitute s.super sub}
end
        ...
        if should_ignore f then []
          ...
          let rec terms = function
            ...
            | Formula.Prop {predicate; args} -> (
                ...
                match List.Assoc.find hint_map predicate ~equal with
                | None -> []
                | Some hints ->
...
module Rule = struct
  ...
  let is_reduction_rule r =
    match r.conclusion with
    | Formula.Prop {predicate; _} -> Predicate.(equal predicate Builtin.step)
    | _ -> false

  let is_typing_rule r =
    match r.conclusion with
   ...
   *        let base = String.subo v ~len:pos in
   *        (t, base, num) :: acc
   *     | _ -> acc)
   * |> List.rev
   * |> List.fold ~init:String.Map.empty
   *      ~f:(fun acc (t, base, num) ->
   *        Map.add_multi acc base (t, num))
   * |> Map.fold ~init:[] ~f:(fun ~key ~data acc ->
   *        let data =
   *          List.sort data ~compare:(fun (_, n) (_, n') ->
   *              Int.compare n n')
   *        in
   *        List.fold data ~init:([], (0, 0))
   *          ~f:(fun (acc, (n, prev_num)) (t, num) ->
   *            match t with
   *            | Term.Var v ->
   *               let n' =
   *                 if num < prev_num
...
module Grammar = struct
  module Category = struct
    ...
    let to_string c =
      Printf.sprintf "%s %s ::= %s" c.name c.meta_var
        ( Set.to_list c.terms
    ...
    let deuniquify_terms c =
      let subs =
        Set.to_list c.terms |> List.map ~f:Term.vars |> List.concat
        |> List.dedup_and_sort ~compare:Term.compare
        |> List.filter_map ~f:(function
             | Term.Var v -> (
                 let f = function
      ...
      in
      let terms =
        Term_set.map c.terms ~f:(fun t -> Term.substitute t subs)
      in
      {c with terms}
  end
...
let to_string lan =
  let relations_str =
    if Map.is_empty lan.relations then ""
    else
      Map.to_alist lan.relations
      |> List.map ~f:(fun (p, ts) ->
             let len = List.length ts in
             if Predicate.(equal p Builtin.typeof) && len = 3 then
               Printf.sprintf "%s |- %s : %s."
               ...
               Printf.sprintf "%s |- %s ==> %s : %s."
                 ...
                 (Term.to_string (List.nth_exn ts 2))
                 (Term.to_string (List.last_exn ts))
             else if Predicate.(equal p Builtin.step) && len = 2 then
               Printf.sprintf "%s --> %s."
                 (Term.to_string (List.hd_exn ts))
                 (Term.to_string (List.last_exn ts))
             else if Predicate.(equal p Builtin.subtype) && len = 2 then
               Printf.sprintf "%s <: %s."
                 (Term.to_string (List.hd_exn ts))
                 (Term.to_string (List.last_exn ts))
               ...
               Printf.sprintf "%s %s." p
                 (List.map ts ~f:Term.to_string |> String.concat ~sep:" "))
      |> String.concat ~sep:"\n"
      |> fun s -> "\n\n%\n\n" ^ s
  in
  let hints_str =
    if Map.is_empty lan.hints then ""
    else
      Map.data lan.hints
...
let kind_of_op lan op =
  ...
  |> List.fold ~init:None ~f:(fun candidate c ->
         ...
         | Some c' ->
             ...
             let ops' = ops_of c' in
             let ops = ops_of c in
             if Set.length ops > Set.length ops' then Some c else candidate)
  |> Option.map ~f:(fun c -> c.name)
...
let is_meta_var_of lan v category_name =
  match Map.find lan.grammar category_name with
  | None -> false
  | Some {name; meta_var; terms} -> String.is_prefix v ~prefix:meta_var

let is_const_var lan v =
  match kind_of_var lan v with
  | None -> false
  | Some name -> (
...
let reduction_rules_of lan =
  Map.data lan.rules |> List.filter ~f:Rule.is_reduction_rule

let typing_rules_of lan =
  Map.data lan.rules |> List.filter ~f:Rule.is_typing_rule
...
let subset_categories lan =
  ...
  |> List.fold ~init ~f:(fun subsets C.{name; meta_var; terms} ->
         let ops =
           ...
           |> String.Set.of_list
         in
         if Set.is_empty ops then subsets
         else
           ...
           |> List.find ~f:(fun c ->
                  let ops' =
                    ...
                    |> String.Set.of_list
                  in
                  Set.length ops < Set.length ops' && Set.is_subset ops ops')
           |> function
           | None -> subsets
