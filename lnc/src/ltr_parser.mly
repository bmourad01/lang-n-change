...
%token MAPSTO
%token EQ
%token QMEMBER QNOTHING QSOMETHING QEMPTY QVAR QSTR QCONSTRUCTOR QBINDING QSUBST QLIST QMAP QTUPLE QVARKIND QOPKIND QSYNTAX QSTARTSWITH QENDSWITH
%token MATCH WITH FRESHVAR SUBSTITUTE VAROVERLAP NTH HEAD TAIL LAST DIFF ASSOC INTERLEAVEPAIRS APPEND REV DEDUP CONCAT VARS UNBIND BOUND NOTHING SOMETHING GET IF THEN ELSE LET REC IN UPPERCASE LOWERCASE INTSTR SELF UNIQUIFY SETSYNTAX REMOVESYNTAX METAVAR SYNTAX ADDRELATION REMOVERELATION RULENAME PREMISES CONCLUSION RULES ADDRULES ADDRULE SETRULES HINT
...
exp:
  ...
  | ADDRELATION LPAREN relation RPAREN
    { $3 }
  | REMOVERELATION LPAREN relation RPAREN
    { $3 }
  | formula
    { Exp.New_formula $1 }
