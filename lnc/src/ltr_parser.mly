...
%token LIFT TO KEEP SKIP MATCH WITH WHEN FRESHVAR SUBSTITUTE VAROVERLAP NTH HEAD TAIL LAST DIFF INTERSECT GETASSOC ASSOC LENGTH APPEND REV DEDUP CONCAT VARS UNBIND BOUND NOTHING SOMETHING GET IF THEN ELSE LET REC IN UPPERCASE LOWERCASE STRINT INTSTR SELF UNIFYNORMALIZE UNIFY UNIQUIFY REMOVESYNTAX METAVAR CATEGORIES SYNTAX OPKIND VARKIND ADDRELATION RELATIONS SETRELATIONS REMOVERELATION RULENAME PREMISES CONCLUSION SETCONCLUSION RULES ADDRULES ADDRULE SETRULES HINT HINTLIST REMOVEHINT
