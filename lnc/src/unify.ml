...
let is_valid t =
  Set.exists t ~f:(fun s ->
      Solution.is_term_sub s || Solution.is_formula_sub s)
  |> not
...
let run ?(normalize = false) state (lan : L.t) =
  let rec fresh vars = function
    | T.Var v ->
        let var = T.Var (v ^ "'") in
        if Set.mem !vars var then fresh vars var
        else (
          vars := Set.add !vars var;
          var )
    | _ -> failwith "unreachable"
  in
  let principal =
    match Map.find lan.hints "principal" with
    | None -> invalid_arg "'principal' hint is required"
    | Some principal -> principal
  in
  let mode =
    match Map.find lan.hints "mode" with
    ...
    | Solution.Candidate F.(Prop {predicate; args}) ->
        let rec matches_pattern ts =
          List.exists ts ~f:(function
            | T.Nil | T.Constructor _ | T.Cons _ -> true
            | T.Tuple ts -> matches_pattern ts
            | _ -> false)
        in
        principals predicate args |> matches_pattern
    | _ -> false
  in
  let rec loop state =
    ...
    | Some (Solution.Term_sub (t1, t2) as tsub) -> (
        ...
        ( match (t1, t2) with
        | T.Var v1, T.Var v2 -> (
          match (L.kind_of_var lan v1, L.kind_of_var lan v2) with
          | None, _ | _, None -> ()
          | Some c1, Some c2 -> (
        ...
        | T.Constructor c1, T.Constructor c2 ->
            if String.equal c1.name c2.name then
              zip_and_loop state' incompat c1.args c2.args
            else incompat ()
        ...
        | T.Tuple ts, T.Tuple ts'
         ...
         |T.Map_union_uniq ts, T.Map_union_uniq ts' ->
            zip_and_loop state' incompat ts ts'
        ...
        | T.Var v1, T.Var v2
          when (not (String.equal v1 v2))
               && L.is_const_var lan v1 && L.is_const_var lan v2 ->
            incompat ()
        | t, (T.Var v as var) when L.is_const_var lan v || not (T.is_var t)
          -> loop (Set.add state' (Solution.Term_sub (var, t)))
        | (T.Var v as var), t ->
            let vars = T.vars t in
            if
              (not (L.is_const_var lan v))
              && not (List.mem vars var ~equal:T.equal)
            then
              let vars =
                Set.to_list state'
                |> List.map ~f:(function
                     ...
                     | Solution.Candidate f | Solution.Proven f -> F.vars f)
                |> List.concat |> L.Term_set.of_list
              in
              ...
              if Set.mem vars var then
                ...
                Solution_set.map state ~f:(function
                  ...
                  | Solution.Proven f -> Solution.Proven (F.substitute f sub))
                |> loop
              else loop state'
            else incompat ()
        | _ -> loop state' )
      ...
      | _ -> (
        ...
        | Some (Solution.Candidate (F.Prop p as f) as candidate) ->
            ...
            let rec prove = function
              ...
              | state' :: rest -> (
                try loop state'
                with Incompatible_terms _ | Incompatible_formulae _ ->
                  prove rest )
            in
            let state =
  ...
  if normalize then
    ...
    let rec loop' state =
      let rec find = function
        ...
        | f :: fs -> (
          match f with
          | Solution.Candidate (F.Prop p) | Solution.Proven (F.Prop p) -> (
              ...
              let rec find' = function
                ...
                | f' :: fs' -> (
                  ...
                  | _ -> find' fs' )
              in
              match find' Set.(to_list (remove state f)) with
              | None -> find fs
              | Some s -> Some s )
          | _ -> find fs )
      in
